...
__global__ void assign_pos_kernel(int *cum_count, const long *gate, long *pos,
...
void fmoe_cuda_assign_pos_impl(int *cum_count, const long *gate, long *pos,
                               const size_t batch_size, const size_t topk,
                               CudaStreamManager *smgr) {
  size_t numel = batch_size * topk;
  assign_pos_kernel<<<CEIL(numel, 256), 256, 0, smgr->torchStream()>>>(
...
__global__ void expert_count_kernel(const long *gate_idx, int *expert_count,
                                    const size_t batch_size,
                                    const size_t n_expert) {
...
void fmoe_cuda_expert_count_impl(const long *gate_idx, int *expert_count,
                                 const size_t batch_size, const size_t n_expert,
                                 CudaStreamManager *smgr) {
  // gate_idx 2048*2 expert_count 32 batch_size 4096 n_expert 32
  expert_count_kernel<<<CEIL(n_expert, PERTHREAD_EXPERTS), 256, 0,
                        smgr->torchStream()>>>(gate_idx, expert_count,
                                               batch_size, n_expert);
}
