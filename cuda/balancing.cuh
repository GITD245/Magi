...
__global__ void limit_by_capacity_kernel(const long *ec, int *cap, long *eca,
                                         const long n_expert,
                                         const long n_worker) {
  int eid = blockIdx.y;
...
void fmoe_cuda_limit_by_capacity_impl(const long *ec, int *cap, long *eca,
...
__global__ void prune_gate_by_capacity_kernel(const long *gate_idx,
                                              long *new_gate_idx, int *ec,
...
void fmoe_cuda_prune_gate_by_capacity_impl(long *gate_idx, long *new_gate_idx,
                                           int *ec, const long batch_size,
                                           const long n_expert,
                                           const long n_worker,
                                           CudaStreamManager *smgr) {
  dim3 grid_dim(CEIL(batch_size, 1024));
  dim3 block_dim(1024);
  prune_gate_by_capacity_kernel<<<grid_dim, block_dim, 0,
                                  smgr->torchStream()>>>(
