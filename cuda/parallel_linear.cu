...
torch::Tensor _linear_forward(torch::Tensor input_buf,
  ...
  if (bias.has_value()) {
    output = bias.value().repeat_interleave(
        expert_count.to(bias.value().device()), 0);
  } else {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      "moe_forward_cuda", ([&] {
        fmoe_cuda_linear_forward_impl<scalar_t>(
            input_buf.data_ptr<scalar_t>(), weight.data_ptr<scalar_t>(),
            expert_count.data_ptr<long>(), output.data_ptr<scalar_t>(),
            bias.has_value(), in_feat, out_feat, num_expert, smgr);
      }));
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      "moe_cuda_backward", ([&] {
        fmoe_cuda_linear_backward_impl<scalar_t>(
            ...
            input_buf.data_ptr<scalar_t>(), weight.data_ptr<scalar_t>(),
            expert_count.data_ptr<long>(), grad_input_buf.data_ptr<scalar_t>(),
            grad_weight.data_ptr<scalar_t>(), grad_bias.data_ptr<scalar_t>(),
            bias.has_value(), batch_size, in_feat, out_feat, num_expert, smgr);
      }));
