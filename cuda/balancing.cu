...
torch::Tensor _limit_by_capacity(torch::Tensor expert_count,
  ...
  fmoe_cuda_limit_by_capacity_impl(
      expert_count.data_ptr<long>(), capacity.data_ptr<int>(),
      expert_count_ack.data_ptr<long>(), n_expert, n_worker, smgr);
...
torch::Tensor _prune_gate_by_capacity(torch::Tensor gate_idx,
  ...
  auto opt =
      torch::TensorOptions().dtype(gate_idx.dtype()).device(gate_idx.device());
  auto new_gate_idx = torch::empty(gate_idx.sizes(), opt);
  fmoe_cuda_prune_gate_by_capacity_impl(
      gate_idx.data_ptr<long>(), new_gate_idx.data_ptr<long>(),
      expert_count.data_ptr<int>(), batch_size, n_expert, n_worker, smgr);
  return new_gate_idx;
}

template <class T> T *_cudamalloc(size_t sz) {
  T *dptr;
  cudaMalloc(&dptr, sz * sizeof(T));
  return dptr;
}

template <class T> T *_h2d(const T *hptr, T *dptr, size_t sz) {
  cudaMemcpy(dptr, hptr, sz * sizeof(T), cudaMemcpyHostToDevice);
  return dptr;
}
template <class T> T *_h2d(T *hptr, size_t sz) {
  T *dptr = _cudamalloc<T>(sz);
  return _h2d(hptr, dptr, sz);
}
template <class T> T *_d2h(const T *dptr, T *hptr, size_t sz) {
  cudaMemcpy(hptr, dptr, sz * sizeof(T), cudaMemcpyDeviceToHost);
  return hptr;
}
template <class T> T *_d2h(const T *dptr, size_t sz) {
  T *hptr = new T[sz];
...
std::vector<torch::Tensor> _swipe_once(torch::Tensor gate_idx,
  ...
  ncclAllGather(d_gcap + rank, d_gcap, 1, ncclInt64, smgr->ncclcomm,
                smgr->torchStream());
  smgr->syncTorch();
