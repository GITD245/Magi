...
class magi_profile():
    def __init__(self,args,window_size=10):
        ...
        self.print_token=True
        self.save_global_token_log=True
        ...
        self.per_itr_record_time={'stime':[0 for _ in range(self.num_layers)],
                                      'ctime':[0 for _ in range(self.num_layers)],
                                      'rtime':[0 for _ in range(self.num_layers)],
                                      'shadow_stime':[0 for _ in range(self.num_layers)],
                                      'shadow_ctime':[0 for _ in range(self.num_layers)]}
    ...
    def lg_token_to_sro_token(self,layer,itr=0):
        origin_token={}
        recive_token={}
        ...
        if self.print_token:
            print(f"itr :{self.itr} recive_token:{recive_token} origin_token:{origin_token}")
        
        return recive_token,origin_token
    ...
    def next_itr(self):
        ...
        self.lg_token_to_sro_token(0,0)
        ...
        if self.magi_profile_flag and self.print_time:
            ...
            print(f"rank:{self.rank} layer:{5} stime:{stime:.2f} ctime:{ctime:.2f} rtime:{rtime:.2f} shadow_stime:{shadow_stime:.2f} shadow_ctime:{shadow_ctime:.2f}\n")
